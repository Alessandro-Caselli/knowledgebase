...
export default function EditDocumentPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast('Documento aggiornato', 'success');
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <p className="text-sm text-gray-500 flex-1">
          Modifica: <span className="font-medium text-gray-900">{form.title}</span>
        </p>
        ...
        <div className="max-w-5xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col gap-4">
              ...
              <Select label="Stato" value={form.status} onChange={e => set('status', e.target.value)}>
                <option value="draft">Bozza</option>
                <option value="published">Pubblicato</option>
                <option value="archived">Archiviato</option>
              </Select>
              <Textarea label="Sommario" value={form.summary} onChange={e => set('summary', e.target.value)} rows={3} placeholder="Breve descrizione..." />
              <Textarea label="Note sulle modifiche" value={form.change_summary} onChange={e => set('change_summary', e.target.value)} rows={2} placeholder="Cosa è cambiato?" />
            </div>

            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-1.5"><Tag size={14} /> Tag</h3>
              <div className="flex gap-2 mb-2">
                <input value={tagInput} onChange={e => setTagInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } }}
                  placeholder="Aggiungi tag..." className="flex-1 text-sm border border-gray-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <button onClick={addTag} className="px-3 py-1.5 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 transition-colors">+</button>
              </div>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {form.tags.map(tag => (
                  <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-xs font-medium">
                    #{tag}
                    <button onClick={() => set('tags', form.tags.filter(t => t !== tag))} className="ml-0.5"><X size={10} /></button>
                  </span>
                ))}
              </div>
            </div>
